...
alias ug="cd ~/.config; git pull origin main; sleep 0.37; ~/bin/SYNC; sleep 0.37; git add skhd karabiner yabai sketchybar linearmouse btop bin_scripts zsh_configs .gitignore README.md readme_old25.md mouseless_gh-backup && git commit -m \"\$(date +'%Y-%m-%d %H:%M:%S')\" && git push origin main; cd ~/qns7.github.io && git pull origin main && git add . && git commit -m \"\$(date +'%Y-%m-%d %H:%M:%S')\" && git push origin main"
